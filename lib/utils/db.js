...
export class Db {
	...
	where(field, value, operator="AND", equal="="){
		if(!this.q_data){
			this.q_data = {};
		}
		...
		if(this.q_where){
			this.q_where += stringFormat` ${operator} ${field} ${equal} ${v}`;
		}else{
			this.q_where = stringFormat`WHERE ${field}${equal}${v}`;
		}
	}
	...
	whereBetween(field, start, end, operator="AND"){
		if(!this.q_data){
			this.q_data = {};
		}

		if(this.q_where){
			this.q_where += stringFormat` ${operator} ${field} between '${start}' and '${end}'`;
		}else{
			this.q_where = stringFormat`WHERE ${field} between '${start}' and '${end}'`;
		}
	}
