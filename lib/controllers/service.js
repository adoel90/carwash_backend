...
export class ServiceController extends Controller {
	...
	*/
	changeServiceTypeStatus(param) {
		return new Promise((resolve, reject) => {
			let serviceModel = new ServiceModel();

			serviceModel.getServiceTypeById(param.id).then((service) => {
				let serviceParam = {
					deleted_at : service.deleted_at ? null : this.moment(new Date).format()
				}
				serviceModel.updateServiceType(param.id, serviceParam).then((type) => {
					return resolve(true);
				}).catch((err) => {
	...
	*/
	updateService(param){
		return new Promise((resolve, reject) => {
			...
			if(param.img) {
				serviceParam.srv_img = this.rewriteImage(param.img);
			}
	...
	*/
	changeServiceStatus(param) {
		return new Promise((resolve, reject) => {
			...
			serviceModel.getServiceById(param.id).then((service) => {
				let serviceParam = {
					deleted_at : service.deleted_at ? null : this.moment(new Date).format()
				}
				serviceModel.updateService(param.id, serviceParam).then(() => {
					return resolve(true);
				}).catch((err) => {
					return reject(err);
				});
		...
		return new Promise((resolve, reject) => {
			...
			memberModel.getMemberById(param.member).then((member) => {
				serviceModel.getServiceById(param.service).then((service) => {
					...
					serviceModel.getServiceQueue(null).then((queue) => {
						let transParam = {
							m_id : param.member,
	...
	*/
	serviceTransactionDetail(param) {
		return new Promise((resolve, reject) => {
			let serviceModel = new ServiceModel();

			serviceModel.getServiceTransactionById(param.id).then((transaction) => {
				let result = this.build.transactionService(transaction);
				result.service = this.build.service(transaction);
				result.service.type = this.build.serviceType(transaction);
				return resolve(result);
			}).catch((err) => {
				return reject(err);
			});
