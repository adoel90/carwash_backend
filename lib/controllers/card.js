	...
	*/
	generateCardId(param) {
		return new Promise((resolve, reject) => {
			...
			for (let i = 0; i < 2; i++) {
				Card.getCardTypeById(param).then((type) => {
					Card.generateCardId(type.ct_id).then((data) => {
						let balance = 0;
						if (type.ct_id === 1) balance = '100000'
						else if (type.ct_id === 2) balance = '10000'
						else if (type.ct_id === 3) balance = '20000'
						...
						Member.insertMember(memberParam).then((member) => {
							let logParam = {
								m_id : member.m_id,
							...
							Log.createLogUser(logParam).then(() => {
								Card.getLastRow('10').then((result) => {
									return resolve(result);
								});
							}).catch((err) => {
								return reject(err);
