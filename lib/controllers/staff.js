...
export class StaffController extends Controller {
                            ...
                            userModel.getUserById(staff[1][i].created_by).then((user) => {
                                ...
                                if(result.data.length >= staff[1].length) {
                                    if(param.print){
                                        ...
                                        for(let i=0; i<staff[1].length; i++){
                                            result.table.data.push([
                                                this.moment(staff[1][i].ts_date).format("DD MMM YYYY hh:mm:ss"),
                                                user.u_name,
                                                staff[1][i].m_name,
                                    ...
                                    if(param.convert) {
                                        ...
                                        for(let i=0; i<staff[1].length; i++){
                                            for (let z = 0; z < result.data[i].item.length; z++) {
                                                let paramConvert = {
                                                    "Tanggal Transaksi" : this.moment(staff[1][i].ts_date).format("DD MMM YYYY hh:mm:ss"),
                                                    "Nama Item" : result.data[i].item[z].name,
                                                    "Harga Item" : this.parseCurrency(result.data[i].item[z].price, true),
                                                    "Total Item" : result.data[i].item[z].quantity,
                                                    "Nama Staff" : user.u_name,
                                                    "Nama Toko" : staff[1][i].store_name
                                                }
    ...
    */
    getReportStaffByTransactionDetail(param) {
        return new Promise((resolve, reject) => {
            let staffModel = new StaffModel();
            let userModel = new UserModel();

            staffModel.getReportStaffByTransactionItem(param.staff, param.store, param.start_date, param.end_date, param.id).then((staff) => {
                let result = {
                    count: staff[0][0].count,
                    data: []
                }
                            ...
                            userModel.getUserById(staff[1][i].created_by).then((user) => {
                                ...
                                if(result.data.length >= staff[1].length) {
                                    if(param.print){
                                        ...
                                        for(let i=0; i<staff[1].length; i++){
                                            result.table.data.push([
                                                ...
                                                staff[1][i].m_name,
                                                staff[1][i].ti_item,
                                                staff[1][i].ti_quantity,
                                                this.parseCurrency(staff[1][i].ti_price, true)
                                            ]);
                                        }
                                    ...
                                    if(param.convert) {
                                        ...
                                        for(let i=0; i<staff[1].length; i++){
                                            let paramConvert = {
                                                "Tanggal" : this.moment(staff[1][i].ts_date).format("DD MMM YYYY hh:mm:ss"),
                                                "Nama Staff" : user.u_name,
                                                "Nama Member" : staff[1][i].m_name,
                                                "Nama Item" : staff[1][i].ti_item,
                                                "Jumlah Item" : staff[1][i].ti_quantity,
                                                "Harga Item" : this.parseCurrency(staff[1][i].ti_price, true)
                                            }
