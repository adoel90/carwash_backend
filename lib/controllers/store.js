				...
				if(param.print){
					...
					for(let i=0; i<store[1].length; i++){
							result.table.data.push([
								...
								this.parseCurrency(store[1][i].ts_total, true)
							]);
					}
				}
				...
				if(param.convert){
					...
					for(let i=0; i<store[1].length; i++){
						let paramTransaction = {
							...
							"Total Harga" : this.parseCurrency(store[1][i].ts_total, true)
						}
	...
	storeReportTransactionsItem(param) {
		return new Promise((resolve, reject) => {
			...
			storeModel.getReportDetailItemByStore(param.store, param.start_date, param.end_date).then((store) => {
				...
				for(let i=0; i<store[1].length; i++) {
					result.store.push(this.build.storeTransactionItem(store[1][i]));
				}
				...
				if(param.print){
					...
					for(let i=0; i<store[1].length; i++){
							result.table.data.push([
								...
								this.parseCurrency(store[1][i].ti_price, true)
							]);
					}
				}
				...
				if(param.convert){
					...
					for(let i=0; i<store[1].length; i++){
						let paramTransaction = {
							...
							"Total Harga" : this.parseCurrency(store[1][i].ti_price, true)
						}
