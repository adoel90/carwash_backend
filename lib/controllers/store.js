...
export class StoreController extends Controller {
        ...
        return new Promise((resolve, reject) => {
          ...
          storeModel.getStoreList(param.id, param.active).then((store) => {
            ...
            if (param.user.ul_name !== 'Owner' || param.user.ul_name !== 'Staff') {
              let storeData = store[1].filter((data, index, self) =>
                index === self.findIndex((s) => (
                  s.store_name === data.store_name
                ))
              );
              
              for(let i = 0; i < storeData.length; i++) {
                result.store.push(this.build.store(storeData[i]));
              }
            } else {
	...
	*/
	changeStoreStatus(param) {
		return new Promise((resolve, reject) => {
			...
			storeModel.getDetailStoreById(param.id).then((store) => {
				let paramStore = {
					deleted_at : store[0].deleted_at ? null : this.moment(new Date).format()
        }
                  ...
                  menuModel.getStoreMenuList(param.store_id, param.name, param.active).then((menu) => {
                        ...
                        if(param.print){
							...
							for(let i=0; i<menu[1].length; i++){
								...
								menu[1][i].mn_name,
								menu[1][i].mn_desc ? menu[1][i].mn_desc : '-',
								this.parseCurrency(menu[1][i].mn_price, true)
								]);
							}
							return resolve(result);
						}
						...
						if(param.convert) {
							...
							for(let i=0; i<menu[1].length; i++){
								let paramConvert = {
								"Nama Produk" : menu[1][i].mn_name,
								"Deskripsi Produk" : menu[1][i].mn_desc ? menu[1][i].mn_desc : '-',
								"Harga" : this.parseCurrency(menu[1][i].mn_price, true)
								}
								
								resultConvert.push(paramConvert);
							}

							if(resultConvert.length >= menu[1].length) {
			...
			for(let i=0; i< param.menu.length; i++){
				...
				let total = 0;
					...
					for(let i=0; i<menu.length; i++){
						...
						let transParam = {
							m_id : member.m_id,
							ts_date : this.moment(new Date).format('YYYY-MM-DD HH:mm:ss'),
							ts_total : total,
							ts_queue : queue,
	...
	*/
	storeReportTransactions(param) {
		return new Promise((resolve, reject) => {
			let storeModel = new StoreModel();

			storeModel.getReportDetailByStore(param.store, param.start_date, param.end_date).then((store) => {
				let result = {
				...
				for(let i=0; i<store[1].length; i++) {
					result.store.push(this.build.storeTransaction(store[1][i]));
				}
				...
				if(param.print){
					...
					for(let i=0; i<store[1].length; i++){
							result.table.data.push([
								...
								this.moment(store[1][i].ts_date).format("DD MMM YYYY"),
								parseInt(store[1][i].ts_total)
							]);
					}
				}
				...
				if(param.convert){
					...
					for(let i=0; i<store[1].length; i++){
						let paramTransaction = {
							...
							"Tanggal Transaksi" : this.moment(store[1][i].ts_date).format("DD MMM YYYY"),
							"Total Harga" : parseInt(store[1][i].ts_total)
						}
	...
	*/
	storeReportTransactionsItem(param) {
		return new Promise((resolve, reject) => {
			let storeModel = new StoreModel();

			storeModel.getReportDetailItemByStore(param.store, param.start_date, param.end_date).then((store) => {
				let result = {
					count: store[0][0].count,
					store: []
				}
				...
				for(let i=0; i<store[1].length; i++) {
					let strItem = this.build.storeTransactionItem(store[1][i]);
				...
				if(param.print){
					...
					for(let i=0; i<store[1].length; i++){
							result.table.data.push([
								...
								// store[1][i].m_name ? store[1][i].m_name : 'Non-Member',
								this.moment(store[1][i].ts_date).format("DD MMM YYYY"),
								store[1][i].mn_name ? store[1][i].mn_name : '-',
								store[1][i].ti_quantity ? store[1][i].ti_quantity : '-',
								parseInt(store[1][i].ti_price)
							]);
					}
				}
				...
				if(param.convert){
					...
					for(let i=0; i<store[1].length; i++){
						let paramTransaction = {
							...
							// "Nama Member" : store[1][i].m_name ? store[1][i].m_name : 'Non-Member',
							"Tanggal Transaksi" : this.moment(store[1][i].ts_date).format("DD MMM YYYY"),
							"Nama Item" : store[1][i].mn_name ? store[1][i].mn_name : '-',
							"Jumlah Item" : store[1][i].ti_quantity ? store[1][i].ti_quantity : '-',
							"Total Harga" : parseInt(store[1][i].ti_price)
						}
