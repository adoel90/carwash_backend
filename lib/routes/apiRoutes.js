...
export class ApiRoutes extends Routes{
	...
	routes(){
		...
		this.app.delete("/member/remove", verifyToken, (res, respo) => {
			memberController.deleteMemberWhereNameIsNull().then((data) => {
		...
		this.app.get("/card/list/generate/id", verifyToken, (req, res) => {
			if (!this.checkParameters(req.query.type)) {
				return this.error(res, 1);
			}

			cardController.generateCardId(req.query.type).then((data) => {
				return this.success(res, data);
			});
		});
		/*--- End Card routes ---*/
