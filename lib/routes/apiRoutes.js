...
export class ApiRoutes extends Routes{
	...
	routes(){
		...
		this.app.get("/saldo/list", verifyToken, (req, res) => {
			...
			saldoController.getAll(param).then((data) => {
				return this.success(res, data);
			}).catch((err) => {
				return this.error(res, err);
			});
		...
		saldoController.createNewBalance(param).then((data) => {
			return this.success(res, data);
		}).catch((err) => {
			return this.error(res, err);
		});
		...
		saldoController.updateBalance(param).then((data) => {
			return this.success(res, data);
		}).catch((err) => {
			return this.error(res, err);
		});
		...
		this.app.delete("/saldo/delete", verifyToken, (req, res) => {
			...
			saldoController.deleteBalance(param).then((result) => {
		...
		this.app.put("/saldo/status", verifyToken, (req, res) => {
			let param = {
			id: req.body.id
			}
			...
			saldoController.changeStatusBalance(param).then((data) => {
			return this.success(res, data);
			}).catch((err) => {
			return this.error(res, err);
			});
